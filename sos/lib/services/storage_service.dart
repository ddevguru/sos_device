...
class StorageService {
  ...
  static double getClapSensitivity() {
    return _prefs?.getDouble(AppConstants.keyClapSensitivity) ?? 68.0;
  }

  // Emergency Contacts Caching (for instant offline & direct SIM SMS)
  static Future<void> saveCachedContacts(List<ContactModel> contacts) async {
    await init();
    final jsonList = contacts.map((c) => c.toJson()).toList();
    await _prefs!.setString(AppConstants.keyCachedContacts, jsonEncode(jsonList));
  }

  static List<ContactModel> getCachedContacts() {
    final raw = _prefs?.getString(AppConstants.keyCachedContacts);
    if (raw == null) return [];
    try {
